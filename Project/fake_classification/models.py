...
class FakeNewsModel:
    ...
    def load(self, path='./save/lstm.ckpt'):
        self.model.load_weights(path)
        print('Weights loaded')

    def save(self, path='./save/lstm.ckpt'):
        self.model.save_weights(path)
        print('Weights saved')


class FakeNewsLogisticRegression:
    def __init__(self, bow, emb_sz=256, max_len=30, lr=0.001):
        self.bow = bow
        self.emb_sz = emb_sz
        self.vocab_sz = len(bow.word_index)
        self.max_len = max_len
    ...
    def load(self, path='./save/logistic.ckpt'):
        self.model.load_weights(path)
        print('Weights loaded')

    def save(self, path='./save/logistic.ckpt'):
        self.model.save_weights(path)
        print('Weights saved')
